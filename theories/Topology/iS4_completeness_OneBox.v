...
unfold Clos' in *. unfold ClosOneBox in *. inversion H2 ; subst.
...
unfold ClosSubform in *. unfold In in *. destruct H5.
...
unfold Weird_Fin_Intersection in *. simpl in *. unfold In in *.
unfold Ci_uset'. unfold Inf_Union. simpl ; unfold In.
exists (Weird_Fin_Intersection (map (Theories' Γ) (map Box (map BoxOne l)))). split ; auto.
- rewrite <- Box_UnBox_BoxOne_id. exists (map BoxOne l).
  repeat split ; auto.
  + intros A HA. unfold In in *. apply in_map_iff in HA. destruct HA. destruct H ; subst.
     ...
     destruct H0. destruct H ; subst. apply in_map_iff in H0.
     destruct H0. destruct H ; subst. simpl. apply H2 ; auto. unfold In ; auto.
     apply in_map_iff. eexists ; auto.
  + intros v Hv. unfold In in *. unfold Weird_Fin_Intersection in *. unfold In in *.
     exists (Weird_Fin_Intersection (map (Theories' Γ) (map Box l))). split ; auto.
     * exists l ; split ; auto.
     * intros u0 Hu0. apply in_map_iff in Hu0. destruct Hu0. destruct H ; subst.
...
- intros t Ht. apply in_map_iff in Ht. destruct Ht. destruct H ; subst.
  ...
  unfold In in *. unfold Theories'. unfold In. simpl. apply wClosed'.
  + apply Id. apply IdRule_I. apply H1.
     apply in_map_iff. exists (Box B) ; split ; auto. apply in_map_iff ; eexists ; auto.
  + apply H2. apply in_map_iff. eexists ; split ; auto.



exists (Weird_Fin_Intersection (map (Theories' Γ) (map BoxOne (map Box l)))). split ; auto.
- exists l. repeat split ; auto.
  + intros v Hv. unfold In in *. unfold Weird_Fin_Intersection in *. unfold In in *.
     unfold Ci_uset'. unfold Inf_Union. unfold In ; simpl.
     exists (Weird_Fin_Intersection (map (Theories' Γ) (map Box l))). split ; auto.
     ...
     * intros u0 Hu0. apply in_map_iff in Hu0. destruct Hu0. destruct H ; subst.
       apply in_map_iff in H0. destruct H0. destruct H ; subst. unfold In.
       unfold Theories'. unfold In. apply wClosed'.
       apply Id. apply IdRule_I. apply Hv. apply in_map_iff. unfold Theories'.

exists x0 ; split ; auto.
       apply H2. apply in_map_iff. eexists ; split ; auto.

       apply UnBox_BoxTwo_Box_prv.
       apply Id. apply IdRule_I. apply Hv. apply in_map_iff. exists x0 ; split ; auto.

; exists (UnBox (BoxOne x0)) ; split ; auto.
       apply in_map_iff. eexists ; split ; auto. apply in_map_iff. eexists ; split ; auto.
       apply H2. apply in_map_iff. eexists ; split ; auto.
  + intros A HA. unfold In in *.
 apply in_map_iff in HA. destruct HA. destruct H ; subst.
     apply in_map_iff in H0. destruct H0. destruct H ; subst. apply in_map_iff in H0.
     destruct H0. destruct H ; subst. rewrite <- BoxOne_UnBox_fixpoint.
     apply In_Clos'_BoxOne. apply Incl_ClosSubform_Clos'. exists (Box x).
