...
Section TopologicalSemantics.
  ...
  Class upset (pos : poset) :=
    {
      uset : Ensemble (@nodes pos);
      is_upset : forall w, (uset w) -> forall x, @reachable pos w x -> (uset x)
    }.
  ...
  Axiom upset_prf_irrel : forall pos (u0 u1 : upset pos), (@uset _ u0 = @uset _ u1) -> u0 = u1.

  Lemma unit_is_upset : forall (pos : poset), forall w, ((fun (x : @nodes pos) => x = x) w) ->
            forall y, @reachable pos w y -> ((fun (x : @nodes pos) => x = x) y).
  Proof.
  intros. simpl in H. simpl ; auto.
  Qed.

  Instance unit_upset (pos : poset) : upset pos :=
      {|
        uset := (fun (x : @nodes pos) => x = x);
        is_upset := unit_is_upset pos
      |}.

  Lemma inter_is_upset : forall (pos : poset) (u0 u1 : upset pos), forall w, ((fun (x : @nodes pos) => (@uset pos u0) x /\ (@uset pos u1) x) w) ->
            forall y, @reachable pos w y -> ((fun (x : @nodes pos) => (@uset pos u0) x /\ (@uset pos u1) x) y).
  Proof.
  intros. simpl in H. simpl. destruct H ; split.
  - apply (@is_upset pos u0 w) ; auto.
  - apply (@is_upset pos u1 w) ; auto.
  Qed.

  Instance inter_upset (pos : poset) (u0 u1 : upset pos) : upset pos :=
      {|
        uset := (fun (x : @nodes pos) => (@uset pos u0) x /\ (@uset pos u1) x);
        is_upset := inter_is_upset pos u0 u1
      |}.
    ...
    Class model :=
      {
        ...
        (* Interior operator *)
        i : (upset pos) -> (upset pos) ;
        i_unit : i (unit_upset pos) = (unit_upset pos) ;
        i_inter : forall u0 u1, @uset _ (i (inter_upset pos u0 u1)) = @uset _ (inter_upset pos (i u0) (i u1)) ;
        i_T : forall u, Included _ (@uset pos (i u)) (@uset pos u) ;
        i_4 : forall u, Included _ (@uset pos (i u)) (@uset pos (i (i u))) ;
  ...
  match φ with
    | Var p => (@uset (@pos M) (val p)) w
    | Bot => False
    | ψ ∧ χ => (forces M w ψ) /\ (forces M w χ)
    | ψ ∨ χ => (forces M w ψ) \/ (forces M w χ)
    | ψ --> χ => forall v, reachable w v -> forces M v ψ -> forces M v χ
    | Box ψ => forall u, (forall v, forces M v ψ <-> (@uset (@pos M) u) v) -> (@uset (@pos M) (i u)) w
  end.
  ...
  - simpl. simpl in H. apply (@is_upset pos _ w) ; auto.
  ...
  - simpl. simpl in H. intros. pose (H u H1).
    apply (@is_upset pos _ w) ; auto.
  Qed.

  Lemma truthset_is_upset : forall M φ, forall w, ((fun (x : @nodes (@pos M)) => forces M x φ) w) ->
            forall y, @reachable (@pos M) w y -> ((fun (x : @nodes (@pos M)) => forces M x φ) y).
  Proof.
  intros. simpl in H. simpl. apply Persistence with (w:=w) ; auto.
  Qed.

  Instance truthset_upset (M : model) φ: upset (@pos M) :=
      {|
        uset := (fun (x : @nodes pos) => forces M x φ);
        is_upset := truthset_is_upset M φ
      |}.

  Lemma inter_conj : forall M u0 u1 w, (@uset (@pos M) (@inter_upset _ u0 u1)) w ->
          ((@uset (@pos M) u0) w) /\ ((@uset (@pos M) u1) w).
  Proof.
  ...
  Lemma i_id_uset_preserv : forall M u0 u1, (forall w, (@uset (@pos M) u0) w <-> (@uset (@pos M) u1) w) ->
          (forall w, (@uset (@pos M) (i u0)) w <-> (@uset pos (i u1)) w).
  Proof.
  ...
  Lemma i_subset_uset_mon : forall M u0 u1, (forall w, (@uset (@pos M) u0) w -> (@uset (@pos M) u1) w) ->
          (forall w, (@uset (@pos M) (i u0)) w -> (@uset pos (i u1)) w).
  Proof.
  intros.
  ...
  Lemma force_Box_interior_truthset : forall φ M w, forces M w (Box φ) <-> (@uset (@pos M) (i (truthset_upset M φ))) w.
  Proof.
  intros. split ; intro.
  - simpl in H. apply H. intros ; split ; intros ; auto.
  - simpl. intros ; auto.
    assert (forall w, (@uset (@pos M) (truthset_upset M φ)) w <-> (@uset pos u) w).
    split ; intros ; apply H0 ; auto.
    pose (i_id_uset_preserv M _ _ H1). apply i0 ; auto.
  Qed.

  Lemma force_truthset : forall φ M w, forces M w φ <-> (@uset (@pos M) (truthset_upset M φ)) w.
  Proof.
  intros. split ; intro.
  - simpl in H. apply H.
  - simpl. intros ; auto.
